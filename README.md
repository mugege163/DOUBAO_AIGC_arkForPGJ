...
**本项目主推 Python 大模型服务**,提供更强大的 AI 能力和更灵活的开发体验。【由火山引擎提供主要服务】
...
**Python 版本: 3.8+ (推荐使用 Python 3.13+)**
...
`rag_llm_server/config.py` 或 `server_python/main.py`

您可以自定义具体场景,并按需配置相关参数。

Demo 中提供了两种 Python 服务端实现:

- **rag_llm_server**: 基于 FastAPI 的 RAG + LLM 服务(推荐)
- **server_python**: 简化版 Python 服务

注意:

- **推荐使用 rag_llm_server**,它提供了更完整的 RAG(检索增强生成)能力和更好的性能
- 配置 AK/SK:在 `.env` 文件中设置火山引擎的访问密钥,可从 https://console.volcengine.com/iam/keymanage/ 获取
- RTC 配置:AppId、AppKey 可从 https://console.volcengine.com/rtc/aigc/listRTC 中获取
- RoomId、UserId 可自定义也可不填,交由服务端生成
- VoiceChat 配置:可参考 https://www.volcengine.com/docs/6348/1558163 中参数描述,完整填写参数内容
- 可通过 [快速跑通 Demo](https://console.volcengine.com/rtc/aigc/run?s=g) 快速获取参数,跑通后点击右上角 `接入 API` 按钮复制相关代码
...
#### 方式一:使用 rag_llm_server(推荐 - 支持 RAG 能力)
...
| 问题                                                                                                                                | 解决方案                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| :---------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 如何使用第三方模型、Coze Bot                                                                                                        | 模型相关配置在 `rag_llm_server/config.py` 或 `server_python/main.py` 中，填写对应官方模型/ Coze/ 第三方模型的参数后，可点击页面上的 "修改 AI 人设" 进行切换。                                                                                                                                                                                                                                                                                                                                                                                                |
...
| 接口调用时, 返回 "Invalid 'Authorization' header, Pls check your authorization header" 错误                                         | Python 服务端配置文件中的 AK/SK 不正确，请检查 `.env` 文件或配置文件                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Python 服务端启动失败                                                                                                               | 检查 Python 版本是否为 3.8+，检查依赖是否正确安装，检查 .env 配置文件是否正确                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
- 2026-04-22
  - **主推 Python 大模型服务**,提供更强大的 AI 能力
  - 新增 rag_llm_server:基于 FastAPI 的 RAG + LLM 服务
  - 新增 server_python:简化版 Python 服务
  - 优化异步处理能力,提升性能
  - 完善错误处理和日志记录
  - 支持更多大模型集成
