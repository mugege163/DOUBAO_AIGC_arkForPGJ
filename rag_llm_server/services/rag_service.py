...
class RagService:
    def __init__(self):
        ...
        # 如果 .env 中未配置，将使用默认值
        self.collection_name = os.getenv("KB_COLLECTION_NAME", "ticket_guide")  # 票管家购票知识库集合名称
    ...
    async def retrieve(self, query: str) -> str:
        ...
        if not self.ak or not self.sk or not self.account_id:
            print(f"⚠️ [票管家RAG] 配置缺失: 请检查 VOLC_AK, VOLC_SK, VOLC_ACCOUNT_ID(当前: {self.account_id})")
            return ""
        ...
        try:
            ...
            if resp.status_code != 200:
                print(f"❌ [票管家RAG] 请求失败: {resp.status_code}, {resp.text}")
                return ""
            ...
            if not contents:
                print(f"⚠️ [票管家RAG] 未检索到匹配的购票知识内容")
                return ""
            ...
            print(f"✅ [票管家RAG] 成功提取 {len(contents)} 条购票知识内容")
            print(f"【传给LLM的票管家购票上下文】:\n{context_text}")
            return context_text
